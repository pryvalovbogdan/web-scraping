...
(async () => {
    ...
    const results = await page.evaluate(() =>
        Array.from(document.querySelectorAll('.site_cell')).map((item) => {
            ...
            const button = item.querySelector<HTMLElement>('div.button.btn-ksenko.btn-light.btn-bright-hover.w-100.fs-10.fs-sm-12.fs-lg-16.fw-500');

            return {
                title: titleElement ? titleElement.innerText.trim() : null,
                img: imgElement ? imgElement.getAttribute('src') : null,
                link: linkElement ? linkElement.href : null,
                buttonSelector: button ? 'div.button.btn-ksenko.btn-light.btn-bright-hover.w-100.fs-10.fs-sm-12.fs-lg-16.fw-500' : null,
            };
        })
    );
